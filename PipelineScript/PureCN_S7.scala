...
object purecn{
    ...
    val fltbam = INPUT(path = "/mnt/storageBig8/work/nguyenma/ctDNA/2020/BGI/PureCN/update_v3/result_PureCN_v1/FilterBamList/out.csv")
    ...
    val target_bed = INPUT(path = "/mnt/storageBig8/work/nguyenma/projects/ctDNA/Oseq/result/ROI4Set7_final.bed")
    ...
    // Step5: PWC segmentation 
    val PWSSeg = NamedMap[BashEvaluate]("PWSSeg")
    ...
    val cov_vcf = REvaluate(table1 = CoverageList,
                            ...
                            names(table1)=c('Key','covfile')
                            table1 <- table1 %>%    
                                filter(!grepl('BDNA', covfile)) %>%
                                mutate(patient = sub('_.*', '', Key))
                            
                            names(table2) = c('Key','vcf')
                            
                            table2 <- table2 %>%
                                filter(!grepl('_recal', vcf)) %>%
                                mutate(patient = sub('_.*', '', Key))

                            table = table1 %>% left_join(table2[,c('vcf','patient')], by = 'patient')
                            ...
                            table.out = data.frame(table) #[-grep('_recal',table$vcf),]
                            """
                    )
    ...
    for ( row <- iterCSV(cov_vcf.table)) {
            ...
            CNVcall(sample) = BashEvaluate(var1 = covfile, var2 = vcffile,
                                        ...
                                        var5 = makePoNfromZeroTPL.out1, //var6 = target_bed, //intersectBed -a @var2@ -b @var6@ -header > @folder1@/temp.vcf
                                        var6 = CombinePon.out1,
                                        script = """
                                            Rscript /mnt/storageBig8/work/nguyenma/ctDNA/2020/pipeline_script/CNVCalling/PureCN/CNVcall.R @var1@ @var2@ \
                                            @var3@ @var4@ @out1@ @out2@ @out3@
                                            
                                            Rscript /mnt/storageBig8/work/nguyenma/ctDNA/2020/pipeline_script/CNVCalling/PureCN/CNVcall.R @var1@ @var2@ \
                                            @var5@ @var4@ @folder1@/logR.csv @folder1@/segment.csv @folder1@/result.rds

                                            Rscript /mnt/storageBig8/work/nguyenma/ctDNA/2020/pipeline_script/CNVCalling/PureCN/CNVcall.R @var1@ @var2@ \
                                            @var6@ @var4@ @folder2@/logR.csv @folder2@/segment.csv @folder2@/result.rds
